    ...
    NavigationStack {
      ...
      .toolbar {
        ToolbarItem(placement: .cancellationAction) {
          ...
          .confirmationDialog("Discard Moment",
                              isPresented: $isShowingCancelConfirmation) {
            Button("Discard Moment", role: .destructive) { dismiss() }
          }
        }
        ToolbarItem(placement: .confirmationAction) {
          Button("Add", systemImage: "checkmark") {
            let newMoment = Moment(title: title,
                                   note: note,
                                   imageData: imageData,
                                   timestamp: .now)
            dataContainer.context.insert(newMoment)
            do {
              _ = dataContainer.badgeManager.unlockBadges(newMoment: newMoment)
...
private extension MomentEntryView {
